  ...
  return (
    <>
      <div className="border rounded-lg border-secondary">
        ...
        <div className="p-4">
            <h4 className="text-sm text-destructive-foreground/60">Units</h4>
            <div className="space-y-1">{course.units.map((unit, unitIndex) => {
                return(
                    <Link key={unit.id} href={`/course/${course.id}/${unitIndex}/0`} className="block underline w-fit">
                     {unit.name}  </Link>
                )
            })}
            </div>
